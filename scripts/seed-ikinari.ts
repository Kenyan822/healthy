...
function markPreviousMenusUnavailable(): number {
  const stmt = db.prepare(`
    UPDATE menus SET is_available = 0 WHERE chain_id = 'ikinari'
  `);
...
function main() {
  ...
  console.log(`⏸️  既存いきなりステーキメニュー ${markedCount}件を非表示に設定`);

  // 2. データ投入（UPSERT: 既存は更新、新規は追加）
  const insertedCount = insertIkinariMenus();
  console.log(`✅ いきなりステーキメニュー ${insertedCount}件をUPSERT`);
