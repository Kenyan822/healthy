...
function deletePreviousRingerhutMenus(): number {
  const deleteStmt = db.prepare(`
    DELETE FROM menus WHERE chain_id = 'ringerhut'
  `);
  const result = deleteStmt.run();
  return result.changes;
}

function insertRingerhutMenus(): number {
...
function main() {
  ...
  // 1. 既存データ削除
  const deletedCount = deletePreviousRingerhutMenus();
  console.log(`🗑️  既存リンガーハットメニュー ${deletedCount}件を削除`);

  // 2. 新規データ投入
  const insertedCount = insertRingerhutMenus();
  console.log(`✅ リンガーハットメニュー ${insertedCount}件を投入`);
  ...
  const totalStmt = db.prepare(
    "SELECT COUNT(*) as count FROM menus WHERE chain_id = 'ringerhut'"
  );
  const total = totalStmt.get() as { count: number };
  console.log(`\n✨ 完了! リンガーハットメニュー合計: ${total.count}件`);
}
