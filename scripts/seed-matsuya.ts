...
function deletePreviousMatsuyaMenus(): number {
  const deleteStmt = db.prepare(`
    DELETE FROM menus WHERE chain_id = 'matsuya'
  `);
  const result = deleteStmt.run();
  return result.changes;
}

function insertMatsuyaMenus(): number {
  ...
  let count = 0;
  for (const menu of matsuyaMenuData) {
...
function main() {
  ...
  // 1. 既存データ削除
  const deletedCount = deletePreviousMatsuyaMenus();
  console.log(`🗑️  既存松屋メニュー ${deletedCount}件を削除`);

  // 2. 新規データ投入
  const insertedCount = insertMatsuyaMenus();
  console.log(`✅ 松屋メニュー ${insertedCount}件を投入`);
  ...
  const totalStmt = db.prepare(
    "SELECT COUNT(*) as count FROM menus WHERE chain_id = 'matsuya'"
  );
  const total = totalStmt.get() as { count: number };
  console.log(`\n✨ 完了! 松屋メニュー合計: ${total.count}件`);
}
