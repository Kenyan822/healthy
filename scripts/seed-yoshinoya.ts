...
function deletePreviousYoshinoyaMenus(): number {
  const deleteStmt = db.prepare(`
    DELETE FROM menus WHERE chain_id = 'yoshinoya'
  `);
...
function main() {
  ...
  // 1. 既存データ削除
  const deletedCount = deletePreviousYoshinoyaMenus();
  console.log(`🗑️  既存吉野家メニュー ${deletedCount}件を削除`);

  // 2. 新規データ投入
  const insertedCount = insertYoshinoyaMenus();
  console.log(`✅ 吉野家メニュー ${insertedCount}件を投入`);
